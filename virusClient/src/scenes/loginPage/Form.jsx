...
const Form = ({onClose}) => {
    ...
    const register = async (values, onSubmitProps) => {
        ...
        if (savedUser) {
            if (savedUser.status === 'error')
                return alert(
                    savedUser.msg ? savedUser.msg : 'server disconnected'
                )
            else {
    ...
    const login = async (values, onSubmitProps) => {
        ...
        if (loggedIn) {
            if (loggedIn.status === 'error')
                return alert(
                    loggedIn.msg ? loggedIn.msg : 'Invalid email or password'
                )
            dispatch(
                setLogin({
                    user: loggedIn.user,
    ...
    const forgotPassword = async (values, onSubmitProps) => {
        ...
        const forgetPassword = await forgetPasswordResponse.json()
        onSubmitProps.resetForm()
        console.log(forgetPassword)
        if (forgetPassword.status === 'success') {
            alert('重設密碼的連結已寄到您的信箱,請查收。')
            setPageType('login')
        } else {
            alert(forgetPassword.msg)
        }
    }
    return (
        <Formik
