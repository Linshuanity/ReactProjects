  ...
  return new Promise((resolve, reject) => {
    connectionPool.getConnection((connectionError, connection) => { // 資料庫連線
      if (connectionError) {
        ...
        const query = isDelete ? `DELETE from subscribes WHERE subscriber_id = ${userId} and subscribed_id = ${friendId}` : `INSERT INTO subscribes VALUES (DEFAULT, ${userId}, ${friendId}, DEFAULT)`;
        console.log(isDelete === false);
        console.log(query);
        connection.query(query, (error, result) => { // User資料表寫入一筆資料
          if (error) {
            console.error('SQL error: ', error);
            reject(error); // 寫入資料庫有問題時回傳錯誤
